#!/usr/bin/env bash
# Verifies installed openvibes-ingest, -distribution, and -admin (run as root).
set -euo pipefail
fail() { echo "FAIL: $*" >&2; exit 1; }
expect_stat() { # PATH MODE OWNER:GROUP
    [[ "$(stat -c '%a %U:%G' "$1")" == "$2 $3" ]] || fail "$1 is $(stat -c '%a %U:%G' "$1"), want $2 $3"
}
for user in openvibes_ingest openvibes_distribution openvibes_admin; do
...
[[ "$(rpm -q --qf '[%{FILENAMES} %{FILEFLAGS:fflags}\n]' openvibes-ingest openvibes-distribution openvibes-admin | grep -c '\.toml cn')" == 3 ]] || fail "configs not noreplace"
